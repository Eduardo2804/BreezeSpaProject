...
var openDays = ["Monday", "Wednesday", "Thursday", "Friday", "Sartuday", "Sunday"];
var openTime = 10;
var closeTime = 21;
...
function load() {
  if(typeof(window.Storage) !== "undefined") window.checked = window.localStorage.getItem("checked");
...
function save() {
  if(typeof(window.Storage) !== "undefined") window.localStorage.setItem("checked", JSON.stringify(window.checked));
}
...
function updateHeader() {
  ...
  for(var i = 0; i < days.length; i++){
    ...
    $("#header").append("<td>" + days[i] + " " + date.getDate() + getOrdinal(date.getDate()) + " " + months[date.getMonth()] + " " + date.getFullYear() +  "</td>");
  }
}
...
for(var i = openTime; i < closeTime; i++){
  ...
  for(var y = 0; y < 7; y++){
    ...
    $("#slot-" + i).append(open ? "<td><input type='checkbox' id='" + identifier + "'></input><label for='" + identifier + "'></div></td>" : "<td>-</td>");
    if(window.checked.indexOf(identifier) != -1) document.getElementById(identifier).checked = true;
  }
}
}
